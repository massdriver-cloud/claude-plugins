...
- To deploy: add the bundle as a component once at the project level (MCP `add_component`), then per environment pin the development channel (MCP `update_instance`, version `latest+dev`) and deploy (MCP `create_deployment` + `get_deployment_logs follow:true`)
